...
def render(df, year: int, region: str) -> None:
    ...
    with right:
        st.markdown('<div class="agro-card">' + card_header(
            "Distribución del Rendimiento (Nacional)"), unsafe_allow_html=True)
        st.plotly_chart(ch.yield_distribution(df),
                         use_container_width=True, config={"displayModeBar": False})
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="agro-card">' + card_header(
            "Rendimiento vs Área Cosechada (escala log)"), unsafe_allow_html=True)
        st.plotly_chart(ch.efficiency_scatter(sub, REGION_COLORS),
                         use_container_width=True, config={"displayModeBar": False})
        st.markdown('</div>', unsafe_allow_html=True)
